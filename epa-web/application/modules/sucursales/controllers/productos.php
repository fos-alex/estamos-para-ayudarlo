...
class productos extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Sucursales.Productos.View');
		$this->load->model('sucursales_model', null, true);
		$this->lang->load('sucursales');
		
		Template::set_block('sub_nav', 'productos/_sub_nav');
	...
	{
		$this->auth->restrict('Sucursales.Productos.Create');
		...
		if (isset($_POST['save']))
		{
			if ($insert_id = $this->save_sucursales())
			{
				...
				redirect(SITE_AREA .'/productos/sucursales');
			}
			else
			{
	...
	public function edit()
	{
		...
		if (isset($_POST['save']))
		{
			$this->auth->restrict('Sucursales.Productos.Edit');
		...
		{
			$this->auth->restrict('Sucursales.Productos.Delete');
